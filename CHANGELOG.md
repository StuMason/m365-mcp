...
- The published package no longer contains test files. `package.json` ships
  `dist`, and `tsc` was emitting `dist/__tests__`, so the compiled test suite was
  part of every tarball. `tsconfig.json` now excludes `src/__tests__` from the
  build.
